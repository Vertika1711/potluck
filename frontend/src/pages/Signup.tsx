...
function Signup() {
  ...
  // NEW: true once signup succeeds -- switches the whole card to a
  // "check your email" confirmation instead of redirecting to /login,
  // since a fresh account can't log in yet until it's verified.
  const [signupSuccess, setSignupSuccess] = useState(false);

  // NEW: true only when the signup failure was specifically "this email
  // already exists but hasn't been verified yet" -- lets us offer a
  // resend option instead of a dead-end duplicate-account error.
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      // UPDATED: no longer navigates to /login -- a brand new account
      // can't log in yet (blocked until the verification email is
      // clicked), so redirecting there would just lead to an immediate,
      // confusing "please verify your email" error. Showing a
      // confirmation state on THIS page instead is clearer.
      ...
      // the "email already used, but that account was never verified"
      // case -- checked here so the resend option only appears then.
      setIsUnverified(err.response?.data?.unverified === true);
    }
  }

  // NEW: calls the same resend-verification route Login.tsx uses, with
  // whatever email is currently typed in the form.
    ...
    <div className="w-screen relative left-1/2 -ml-[50vw] min-h-screen flex flex-col bg-[#efe0c0] font-sans">
      ...
      <div className="flex-1 flex items-center justify-center px-4 py-4">
        ...
        <div className="w-full max-w-sm sm:max-w-md bg-white/60 backdrop-blur-sm rounded-lg p-5 sm:p-6">
          {signupSuccess ? (
            // NEW: replaces the whole form with a confirmation message
            // once signup succeeds -- same "hide the form entirely once
            // done" pattern as ResetPassword.tsx's success state.
            <div className="flex flex-col items-center text-center gap-3">
              ...
              <p className="text-[#4a3620]">
                We've sent a verification link to <strong>{email}</strong>. Click it to activate your account, then come back and log in.
              </p>
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col">
                ...
                {/* NEW: only shown when the signup error was specifically
                    a duplicate-but-unverified account -- same "resend"
                    pattern as Login.tsx, so someone who mistyped or lost
                    their first verification email isn't stuck. */}
