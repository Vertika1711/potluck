...
const UNFINISHED_STATUSES: ISwap["status"][] = ["pending", "pending_share", "pending_pick", "pending_confirmation"];
const EXPIRY_DAYS = 7;
...
async function expireIfOverdue(swap: HydratedDocument<ISwap>): Promise<HydratedDocument<ISwap>> {
...
function actingUserId(swap: HydratedDocument<ISwap>): string {
...
// NEW: PUT /api/swaps/:id/share — Case A only. The want-listing owner
...
// NEW: PUT /api/swaps/:id/pick — whoever's turn it is (see actingUserId)
...
router.put("/:id/pick", requireAuth, async (req: AuthRequest, res) => {
  try {
    const { action, selectedListingId } = req.body; // action: "pick" or "reject"
    ...
    if (action !== "pick") {
      return res.status(400).json({ error: "action must be 'pick' or 'reject'." });
    }
...
// NEW: PUT /api/swaps/:id/confirm — the final accept/reject once a pick
